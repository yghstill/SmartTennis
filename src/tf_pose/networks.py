...
from src.tf_pose.network_mobilenet import MobilenetNetwork
from src.tf_pose.network_mobilenet_thin import MobilenetNetworkThin

from src.tf_pose.network_cmu import CmuNetwork
from src.tf_pose.network_personlab import PersonLabNetwork


def _get_base_path():
    if not os.environ.get('OPENPOSE_MODEL', ''):
        return '../../model/tf_pose'
...
def get_graph_path(model_name):
    dyn_graph_path = {
        'cmu': './model/tf_pose/graph/cmu/graph_opt.pb',
        'mobilenet_thin': './model/tf_pose/graph/mobilenet_thin/graph_opt.pb'
    }
