...
def arg_parse():
    ...
    parser.add_argument("--width", dest = "width", help = "video width", default = 1280)
    parser.add_argument("--height", dest = "height", help = "video height", default = 720)
...
def handle_one(oriImg):
    ...
    print('handle_one'+str(oriImg.shape))
...
def write(x, img,classes,colors):
    c1 = tuple(x[1:3].int())
    c2 = tuple(x[3:5].int())
    #print(str(x[1].int()) + " " + str(x[2].int()) + "  bottom:" +str(x[3].int()) + " " +str(x[4].int()))
    cls = int(x[-1])
...
def handel_two(ret, frame, inp_dim,model,num_classes,classes,colors):
    ...
    #bbox_img = orig_im
    #bbox_im = list(map(lambda x:bbox_img[x[1]:x[2],x[3]:x[4],:],output))   
    list(map(lambda x: write(x, orig_im,classes,colors), output))
    return True, orig_im, output
########################

def update_img_bbox(x, orig_img,classes):
    cls = int(x[-1])
    label = "{0}".format(classes[cls])
    if label == 'person':
        print(list(x))
        ...
        bbox_img = orig_img[x[2].int():x[4].int(),x[1].int():x[3].int(),:]
        #cv2.imshow('a',bbox_img)
        #time.sleep(2)
        print(bbox_img.shape)
        bbox_img = handle_one(bbox_img)
        orig_img[x[2].int():x[4].int(),x[1].int():x[3].int(),:] = bbox_img
    return orig_img
...
if __name__ == "__main__":
    ...
    out = cv2.VideoWriter(outname,fourcc, 25, (video_width,video_height))
    frames = 0
    start = time.time()
    #指定video文件
    ...
    while video_capture.isOpened():
        ...
        if not ret:
            break
        type_, orig_img,output = handel_two(ret, frame, inp_dim,yolo_model,num_classes,classes,colors)
        if not type_:
            frames += 1
            FPS = 'FPS:{:5.2f}'.format(frames / (time.time() - start))
            cv2.putText(orig_img, FPS, (20,20), cv2.FONT_HERSHEY_PLAIN, 1, [0,255,0], 1)
        ...
        canvas = handle_one(orig_img[59:635,252:1082,:])
        orig_img[59:635,252:1082,:] = canvas
        #list(map(lambda x: update_img_bbox(x, orig_img,classes), output))
        FPS = 'FPS:{:5.2f}'.format(frames / (time.time() - start))
        print(FPS)
        cv2.putText(orig_img, FPS, (20,20), cv2.FONT_HERSHEY_PLAIN, 1, [0,255,0], 1)
        # Display the resulting frame
        #cv2.imshow('Video', orig_img)  #显示视频
        out.write(orig_img)  #保存视频
